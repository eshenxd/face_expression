...
int FaceDetector::runFaceDetector()
{
	...
	//double param[4] = { 1.2, 1.4, 1.6, 2.0 };
	//while (i < 4)
	//{
	//	faces = cvHaarDetectObjects(small_img, cascade_faces, storage_faces,
	//		param[i], 3, 0/*CV_HAAR_DO_CANNY_PRUNING*/,
	//		cvSize(50, 50));
...
void FaceDetector::drawDetectedFaces(IplImage* image_draw, CvPoint p1, CvPoint p2, CvScalar color)
{
...
void FaceDetector::drawRect(IplImage* img ,CvPoint p1 ,CvPoint p2){
	...
	cvLine(img,top_left,cvPoint(top_left.x+dis_long_x,top_left.y),cvScalar(0,0,255),1,8,0);
	cvLine(img,top_left,cvPoint(top_left.x,top_left.y+dis_long_y),cvScalar(0,0,255),1,8,0);
	cvLine(img,top_right,cvPoint(top_right.x,top_right.y+dis_long_y),cvScalar(0,0,255),1,8,0);
	cvLine(img,top_right,cvPoint(top_right.x - dis_long_x,top_right.y),cvScalar(0,0,255),1,8,0);

	cvLine(img,bottom_left,cvPoint(bottom_left.x + dis_long_x,bottom_left.y),cvScalar(0,0,255),1,8,0);
	cvLine(img,bottom_left,cvPoint(bottom_left.x,bottom_left.y-dis_long_y),cvScalar(0,0,255),1,8,0);
	cvLine(img,bottom_right,cvPoint(bottom_right.x,bottom_right.y-dis_long_y),cvScalar(0,0,255),1,8,0);
	cvLine(img,bottom_right,cvPoint(bottom_right.x - dis_long_x,bottom_right.y),cvScalar(0,0,255),1,8,0);
